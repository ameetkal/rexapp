...
interface UserTagInputProps {
  ...
  // Simplified multi-user mode (for "Experienced With")
...
export default function UserTagInput({
  ...
  // If using simplified mode, map to internal props
  const effectiveTaggedUsers = selectedUsers || taggedUsers;
  const effectiveOnAddUser = onUsersChange 
    ? (user: TaggedUser) => onUsersChange([...effectiveTaggedUsers, user])
    : onAddUser;
  const effectiveOnRemoveUser = onUsersChange
    ? (userId: string) => onUsersChange(effectiveTaggedUsers.filter(u => u.id !== userId))
    : onRemoveUser;
      ...
      return results
        .filter(user => {
          ...
          if (singleUser) {
            ...
            return !effectiveTaggedUsers.some(tagged => tagged.id === user.id);
          }
        })
  ...
  const handleSelectUser = (user: TaggedUser) => {
    if (singleUser) {
      ...
      if (maxUsers && effectiveTaggedUsers.length >= maxUsers) {
        return;
      }
      
      if (effectiveOnAddUser) {
        effectiveOnAddUser(user);
      }
      setInputValue('');
    }
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {effectiveTaggedUsers.map((user) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => effectiveOnRemoveUser?.(user.id)}
                className="ml-1 inline-flex items-center justify-center w-4 h-4 text-blue-600 hover:text-blue-800"
              >
                <XMarkIcon className="w-3 h-3" />
              </button>
          ...
          <input
            ...
            onKeyPress={handleKeyPress}
            onFocus={() => inputValue.length >= 2 && setShowDropdown(true)}
            placeholder={effectiveTaggedUsers.length === 0 && taggedNonUsers.length === 0 ? placeholder : ""}
            className="flex-1 min-w-[120px] outline-none text-sm placeholder:text-gray-500"
          />
        </div>
      </div>
